...
router.post('/createteam', fetchuser, async (req, res) => {
    try {
        ...
        if (!name || !description) {
            return res.status(422).json({ success: false, message: "Invalid Input", code: "ii" });
        }
        let code = otp_gen.generate(6, { upperCaseAlphabets: true, specialChars: false, lowerCaseAlphabets: true });
        while (await Team.findOne({ code })) {
            code = otp_gen.generate(6, { upperCaseAlphabets: true, specialChars: false, lowerCaseAlphabets: true });
        }//if code is already taken by another team
        const team = await Team.create({ name, description, code });
        const userteam = await UserTeam.create({ userId: req.data.id, teamId: team.id });
        return res.status(200).json({ success: true, message: "team created", team, userteam });
    } catch (error) {
...
router.post('/jointeam', fetchuser, async (req, res) => {
    try {
        ...
        if (!team) {
            return res.status(404).send({ success: false, message: "Incorrect code", code: "ic" });
        }
        let userteam = await UserTeam.findOne({ userId, teamId: team.id });
        if (userteam) {
            return res.status(400).send({ success: false, message: "User already in team", code: "uait" });
        }
...
router.put('/updateteambyid', async (req, res) => {
    try {
        let newteam = {};
        if (req.body.name) {
            newteam = await Team.findByIdAndUpdate(req.body.id, { name: req.body.name }, { new: true });
        }
        if (req.body.description) {
            newteam = await Team.findByIdAndUpdate(req.body.id, { description: req.body.description }, { new: true });
        }
